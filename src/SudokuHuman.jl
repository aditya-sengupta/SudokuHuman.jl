    ...
    function solve(s::Sudoku; depth=0, maxdepth=3)
        ...
        while last_num_found < 81
            ...
            if sum(vec(solution.grid) .> 0) == last_num_found # we didn't find anything new this iteration
                 if depth < maxdepth
                    return solve_guess(solution, flags, depth, maxdepth)
                 else
                    return solution
                 end
            end
        ...
        @assert check(solution; verbose=false)
        # the only way you should get here without causing an error further up should be if you've got a completed puzzle
        solution
    end

    function solve_guess(s::Sudoku, flags::AbstractArray{Bool, 3}, depth, maxdepth)
        if check(s; verbose=false)
            return s
        end
        nums_poss = vec(sum(flags, dims=3)[:,:,1])
        ...
        where_to_guess = where_to_guess[start_sort:end]
        guessed_sudoku = copy(s)
        for idx in where_to_guess
            i, j = (idx - 1) % 9 + 1, ((idx - 1) ÷ 9) + 1
            possible_guesses = findall(flags[i, j, :])
            if length(possible_guesses) < 2
                continue # hack, you shouldn't get this, but I am
            end
            for guess in possible_guesses             
                guessed_sudoku.grid[i, j] = guess
                try
                    solution = solve(guessed_sudoku; depth=depth+1, maxdepth=maxdepth)
                    if check(solution; verbose=false)
                        return solution 
                    end
                catch 
                    guessed_sudoku.grid[i, j] = 0
                end
            end
        end
